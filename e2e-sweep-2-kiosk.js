...
async function run() {
  ...
  await R.check('arrivals grid has 4 cards (only people still inside)', async () => {
    // recentActivity has 3 rows but Rahul Mehta already checked out (TimeOUT
    // 05:30 PM), so he must not appear in a grid that means "on site".
    // Remaining: Priya + Visitor A, plus Visitor B (stayover) and the
    // same-named second visitor from activeVisitorList.
    const count = await page.locator('.a-card').count();
    if (count !== 4) throw new Error(`Expected 4 cards, got ${count}`);
  });

  await R.check('someone who already checked out is not listed', async () => {
    const txt = await page.locator('#arrivalsGrid').textContent();
    if (/Rahul/.test(txt)) {
      throw new Error('a checked-out person is shown as still on site');
    }
  });

  await R.check('two visitors sharing a name are two cards', async () => {
    // Deduping on display name dropped the second one — the reported bug.
    const ids = await page.evaluate(() =>
      [...document.querySelectorAll('#arrivalsGrid .a-card')].map(c => c.dataset.emp)
    );
    if (!ids.includes('VIS-SAMENAME')) {
      throw new Error('same-named visitor collapsed into the first: ' + JSON.stringify(ids));
    }
  });
  ...
  await R.check('a visitor who stayed overnight still appears', async () => {
    ...
    // checked in yesterday and never checked out was counted but invisible —
    // the tile said 2, the grid showed 1.
    // Cards show the FIRST name only, so match on the id instead — both
    // visitors are called "Visitor" on screen.
    const ids = await page.evaluate(() =>
      [...document.querySelectorAll('#arrivalsGrid .a-card')].map(c => c.dataset.emp)
    );
    if (!ids.includes('VIS-STAYOVER')) {
      throw new Error('yesterday\'s still-inside visitor missing from the grid: ' + JSON.stringify(ids));
    }
  });

  await R.check('grid card count matches the active-visitor KPI', async () => {
    const shown = await page.evaluate(() =>
      [...document.querySelectorAll('#arrivalsGrid .a-card')]
        .filter(c => /^VIS/.test(c.dataset.emp || '')).length
    );
    if (shown !== 3) throw new Error(`expected 3 visitor cards, grid shows ${shown}`);
  });
