...
/**
 * Open the dashboard with the mock's getDashboardData forced to
 * holiday:<flag>. Mirrors e2e-lib.js's openPage() html-injection pattern,
 * but patches MOCK_DASHBOARD.holiday in the injected mock script before
 * loading, since openPage() itself has no override hook for mock data.
 */
async function openDashboardWithHoliday(browser, flag) {
  const context = await browser.newContext({ viewport: { width: 1280, height: 720 } });
  const page = await context.newPage();

  const filePath = path.join(__dirname, 'src', 'pages', 'dashboard.html');
  let html = fs.readFileSync(filePath, 'utf8');
  html = html.replace(/<\?=\s*[^?]+\?>/g, '"__GAS_TEMPLATE__"');
  html = html.replace(/<\?\s*[^?]+\?>/g, '');
  html = html.replace(/__GAS_TEMPLATE__/g, '""');

  const patchedMock = GAS_MOCK_SCRIPT.replace('holiday: false,', 'holiday: ' + flag + ',');
  html = html.replace('<head>', '<head><script>' + patchedMock + '</script>');
...
async function run() {
  ...
  // ── 6g: Holiday banner ────────────────────────────────────────────────────
  {
    const R = makeRunner('6g · Dashboard — Holiday banner');

    {
      const { page, context } = await openDashboardWithHoliday(browser, true);
      await settle(page, 300);

      await R.check('holiday banner is visible when data.holiday=true', async () => {
        if (!await page.locator('#holidayBanner').isVisible())
          throw new Error('#holidayBanner not visible with holiday:true');
      });
    ...
    {
      const { page, context } = await openDashboardWithHoliday(browser, false);
      await settle(page, 300);

      await R.check('holiday banner is hidden when data.holiday=false', async () => {
        if (await page.locator('#holidayBanner').isVisible())
          throw new Error('#holidayBanner visible with holiday:false');
      });
