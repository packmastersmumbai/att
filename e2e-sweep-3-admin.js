...
async function run() {
  ...
  // ── 3c: Config tab ────────────────────────────────────────────────────────
  {
    ...
    await R.check('holiday picker shows ≥3 disabled national checkboxes', async () => {
      await settle(page, 200);
      const disabledCount = await page.locator('#holidayList .holiday-check[disabled]').count();
      if (disabledCount < 3) throw new Error(`Only ${disabledCount} disabled national checkboxes`);
    });

    await R.check('holiday picker shows a tickable (non-national) checkbox', async () => {
      const enabledCount = await page.locator('#holidayList .holiday-check:not([disabled])').count();
      if (enabledCount < 1) throw new Error('No tickable holiday checkboxes found');
    });

    await R.check('ticking a holiday and saving shows a success toast', async () => {
      const box = page.locator('#holidayList .holiday-check:not([disabled])').first();
      await box.check();
      await page.click('button[onclick="saveHolidays()"]');
      await page.waitForSelector('#dsToast.show.toast-success', { timeout: 5000 });
      const txt = await page.locator('#dsToast').textContent();
      if (!txt.includes('saved')) throw new Error(`Unexpected toast: "${txt}"`);
    });
